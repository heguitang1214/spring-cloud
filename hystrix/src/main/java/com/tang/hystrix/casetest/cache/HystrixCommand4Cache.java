...
public class HystrixCommand4Cache extends HystrixCommand<Boolean> {
    ...
    protected Boolean run() {
        ...
        // 测试遇到降级，还会不会走缓存？
        // 【依旧会走缓存，缓存不会管结果是正常逻辑拿到的，还是降级逻辑拿到的数据】
        // command2b.execute()会拿到返回值，这个返回值是run()的返回值或者是getFallback()的返回值
//        int i = 1 / 0;
//        return true;
    }

    /**
     * 构建cache的key，【判断请求是不是走缓存】
    ...
    public static void main(String[] args) {
        testHystrixCommand4Cache();
    }

    //    public static class UnitTest {
//        @Test
    private static void testHystrixCommand4Cache() {
        //同一个请求上下文中：需要开启
